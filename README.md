...
&emsp;&emsp; __FAISS & ScaNN__ - When we are dealing with finding similar rows in our data and we have a very high dimensional representation of a vector, with traditional algorithms it takes a long time to find similarly if we use metrics such as euclidean distance and cosine similarity. Facebook Research has launched FAISS that simplifies the process of searching similarity and clustering of dense vectors. In the same light, Google AI Research came up with ScaNN which speeds up the overall process of computing the distance between various features and reduces the time complexity to a large extent. The outputs from these models contain vectors that represent a large amount of information from the input data. 
