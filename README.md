...
&emsp;&emsp; 📃 [__Common Reasons why Machine Learning Projects Fail__](https://medium.com/towards-data-science/common-reasons-why-machine-learning-projects-fail-62fe811024d6) - In order to build and deploy machine learning models, there are certain challenges that must be addressed so that we are hassle-free when trying to build them. Addressing these challenges can be the primary goal of an ML practitioner. In this article, we deal with some of the common ways at which machine learning projects fail. Though it is impressive to get the models to get the best predictions, not taking care of few details can have an impact in the long run. Furthermore, this article also lists various steps that can be taken to reduce the occurrence of failure. 
